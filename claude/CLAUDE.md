...
NEVER modify the following files without explicit user confirmation:
- `~/.claude/settings.json`
- `~/.claude/settings.local.json`
- `~/.claude/CLAUDE.md`
- Any file under `~/.claude/hooks/`
- Any file under `~/.claude/hud/`

All other files can be modified freely without asking.

## Multi-Agent Research: Context Management Protocol

When orchestrating multi-agent research (literature reviews, brainstorming, deep research, etc.), follow these rules to prevent context window overflow.

### Rule 1: Write to File, Return Structured Summary Only

Every research agent MUST write full output to a file and return only a structured summary to the main conversation.

**Mandatory instruction to append to all research agent prompts:**
```
CONTEXT MANAGEMENT: Write your complete output to [target file path].
Return to the main conversation ONLY a structured summary:
1. File path written to
2. Status (complete/partial)
3. Key findings (3-5 bullet points, 1-2 sentences each)
4. Unexpected discoveries or cross-topic connections (if any)
5. Identified gaps or follow-up research needs
Target: 300-500 tokens max. DO NOT include full report content in your return message.
```

### Rule 2: Synthesis Agents Must Read Source Files Directly

The synthesis/integration agent MUST read the full source files via the Read tool. It should NEVER rely on the summaries in the main conversation for deep integration. Summaries exist only for orchestration decisions (e.g., whether to launch follow-up research, adjust direction).

### Rule 3: Background Execution for Parallel Research

When launching 3+ independent research agents, use `run_in_background: true`. Check results via Read tool only when needed for synthesis or coordination.

### Rule 4: Standardized Research Output Structure

All research outputs go to: `{project_root}/.omc/research/{topic}/`
```
.omc/research/{topic}/
├── stages/          # Individual agent outputs
├── reports/         # Synthesis and final reports
...
|----|------|---|-------|------|
| #949 | 11:40 AM | 🔵 | Claude local settings configuration explored | ~304 |
| #948 | 11:39 AM | 🔵 | Claude Code settings reveal advanced configuration with hooks and plugins | ~410 |
</claude-mem-context>